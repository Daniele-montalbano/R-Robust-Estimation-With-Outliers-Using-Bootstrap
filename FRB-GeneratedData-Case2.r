##CASE 2
##regression coefficient that is equal to 0.3 and statistically non-significant which due to
##the effect of 3 outliers has been distorted becoming statistically significant, equal to 0.834
...
plot(x2[1:length(x)],y2[1:length(x)], main="linear regression model",pch=19, xlab="x", ylab="y", xlim=c(-10,25),ylim=c(-40,55))
points(x2[102:104],y2[102:104], col=4, pch=19)
lines(x,out1$fitted, col=2, lwd=2)
lines(x2,out2$fitted, col=4, lwd=2)
legend(-11,57,legend=c("original regression","regression with outliers"),
...
##Bootstrap 2: bootstrap del campione rigenerando le posizioni

boot.camp<-function(y2,x2,beta_st,B=50,seed=320){
	n<-length(y2)
	y.b<-matrix(0,B,n)
	x.b<-matrix(0,B,n)
	intercept<-matrix(0,B)
...
plot(x2[1:length(x)],y2[1:length(x)], main="linear regression model",pch=19, xlab="x", ylab="y", xlim=c(-10,25),ylim=c(-30,55))
points(x2[102:104],y2[102:104], col=4, pch=19)
lines(x,out1$fitted, col=2, lwd=2)
lines(x2,out2$fitted, col=4, lwd=2)
abline(boot1$mean.intercept,boot1$mean.beta, col=3, lwd=2)
abline(boot2$mean.intercept,boot2$mean.beta, col=6, lwd=2)
legend(-11,58,legend=c("original regression","regression with outliers","bootstrap errors estimate","bootstrap resampling estimate"),
...
legend(2,-23,legend=c("classic estimate","FRB S- estimate","FRB MM- estimate"),
lwd=2,lty=c(1,1), col=c(2,3,6))


plot(x2[1:length(x)],y2[1:length(x)], main="estimated regression models",pch=19, xlab="x", ylab="y", xlim=c(-10,25),ylim=c(-40,55))
points(x2[102:104],y2[102:104], col=4, pch=19)
lines(x,out1$fitted, col=2, lwd=2)
lines(x2,out2$fitted, col=4, lwd=2)
lines(x2,resS$fitted.values, col=3, lwd=2)
lines(x2,resMM$fitted.values, col=6, lwd=2)
legend(11,-17,legend=c("classic estimate","distorted estimate","FRB S- estimate","FRB MM- estimate"),
lwd=2,lty=c(1,1), col=c(2,4,3,6))
